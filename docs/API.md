# libcloudph++ API Reference
<small> generated by Claude 4.5 and revised by [@AgnieszkaMakulska](https://github.com/AgnieszkaMakulska) </small>

This document provides a reference for the libcloudph++ programming interface, covering the three microphysics schemes and their usage patterns.
...
- Ice nucleation (`opts.ice_nucl`)
- Ice depositional growth (`opts.depo`)
...
void diag_ice_a_rng(const real_t &a_min, const real_t &a_max); // ice equatorial radius [m]
void diag_ice_c_rng(const real_t &c_min, const real_t &c_max); // ice polar radius [m]
```
...
- `a_min`, `a_max`, `c_min`, `c_max`: Ice semi-axis range in meters
...
void diag_ice_a_rng_cons(const real_t &a_min, const real_t &a_max);
void diag_ice_c_rng_cons(const real_t &c_min, const real_t &c_max);
```
...
**Example**:
```cpp
// Select liquid droplets with wet radius 0.5-1 μm AND kappa 0.1-0.2
particles->diag_water();
...
void diag_ice_a_mom(const int &k); // k-th moment of ice equatorial radius
void diag_ice_c_mom(const int &k); // k-th moment of ice polar radius
```
...
```cpp
void diag_wet_mass_dens(const real_t &r_min, const real_t &r_max); // total mass density of droplets in specified wet radius range
void diag_ice_mix_ratio(); // ice mixing ratio [kg/kg]
```
